...
// Student Name: SANJEEVANI CHAUDHARI
...
void setThreadPriority(_fn fn, uint8_t priority)
{
    bool ok;
    tcb[1].pid = fn;
    ok &= createThread(fn,"LengthyFn",priority);
...
void svCallIsr()
{
    ...
    case 102:
        ...
        //if(pi_bit == 0)
        {
            ...
            else if(step7semaphore->count == 0)
            {
                tcb[taskCurrent].state = STATE_BLOCKED;
                uint8_t k;
                for(k=0;k<MAX_QUEUE_SIZE-1;k++)
                {
       ...
       /* else if(pi_bit == 1)
        {
            if(step7semaphore->count == 0)
            {
                if((step7semaphore->processQueue != 0) && (step7semaphore->queueSize < MAX_QUEUE_SIZE))
                {
                    uint8_t i;
                    ...
                    for (i = 0; i < MAX_TASKS; i++)
                    {
                        if (tcb[i].priority > tcb[taskCurrent].priority)
                        {
                            ...
                            tcb[i].skipcount = tcb[taskCurrent].priority + 8;
                            step7semaphore->processQueue[step7semaphore->queueSize] = taskCurrent;
                            step7semaphore->queueSize++;
                            tcb[taskCurrent].state = STATE_BLOCKED;
                            step7semaphore->queueSize = step7semaphore->queueSize % 10;
                        }
                    }
                }
                NVIC_INT_CTRL_R |= NVIC_INT_CTRL_PEND_SV;
    ...
    case 103:
       ...
       /* else if(pi_bit == 1)
        {
            step7semaphore->count++;
            if(step7semaphore->processQueue[0]>0)
                        {
                            tcb[step7semaphore->processQueue[0]].state = STATE_READY;
                            step7semaphore->queueSize--;
                            //tcb[step7semaphore->processQueue[0]].semaphore = step7semaphore;
                            ...
                            for (i = 0; i < 9; i++)
                                 {
                                  step7semaphore->processQueue[i] = step7semaphore->processQueue[i + 1];
